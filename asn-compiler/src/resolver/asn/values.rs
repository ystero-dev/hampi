    ...
    match referenced_value {
        None => {
            match typeref {
                ...
                _ => Err(resolve_error!("resolve_value: Not Implemented!").into()),
            }
        }
        Some(ref_value) => match ref_value {
            ...
            _ => Err(resolve_error!("{} Not a Referenved Value!", value).into()),
        },
    }
}
...
//
//
fn resolve_object_identifier_value(value: &str, _resolver: &Resolver) -> Result<Vec<u32>> {
    ...
    for component in parsed_oid.components {
        if component.name.is_some() {
            ...
            if num.is_none() {
                return Err(resolve_error!(
                    "Name '{}' not found. Named component other than Well known named components not supported yet!", name).into());
            }
